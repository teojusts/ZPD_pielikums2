...
def scrape_price_and_address():
    try:
        ...
        match = re.search(r"(no|from)?\s*(\d{1,3}(?:[.,]?\d{3})*(?:[.,]\d+)?)\s*(eiro|eur|€|euro|neto|bruto)", price_text, re.IGNORECASE)
...
def perform_initial_selections():
    try:
        ...
        # 3. Click the second dropdown to select and type "Šampeteris"
        ...
        dropdown_element_2.send_keys("Šampeteris")
...
def scrape_all_pages(start_page=1, max_pages=4):
    ...
    # Perform the initial selections (type 'r' and 'Šampeteris')
    ...
    with open("Sampeteris_vakances.json", "w", encoding="utf-8") as f:
