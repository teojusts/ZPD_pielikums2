...
def scrape_price_and_address():
    try:
        ...
        match = re.search(r"\b(\d{1,3}(?:[.\s]?\d{3})*(?:[.,]\d{2})?)\s*(EUR|€|euro|eiro)", price_text, re.IGNORECASE)
...
def perform_initial_selections():
    try:
        ...
        # 3. Click the second dropdown to select and type "Rumbula"
        ...
        dropdown_element_2.send_keys("Rumbula")
...
def scrape_all_pages(start_page=1, max_pages=4):
    ...
    # Perform the initial selections (type 'r' and 'Rumbula')
    ...
    with open("Rumbula_vakances.json", "w", encoding="utf-8") as f:
