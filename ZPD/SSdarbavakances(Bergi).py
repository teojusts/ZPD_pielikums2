...
def set_page_zoom(zoom_level):
    """
    Adjust the zoom level of the current page.
    :param zoom_level: A float representing the zoom level (e.g., 1.0 for 100%, 0.75 for 75%, 1.25 for 125%).
    """
    try:
        driver.execute_script(f"document.body.style.zoom='{zoom_level}'")
        time.sleep(1)  # Allow the page to adjust
        print(f"Set page zoom to {zoom_level * 100}%")
    except Exception as e:
        print("Error setting page zoom:", e)
...
def perform_initial_selections():
    try:
        ...
        # 3. Click the second dropdown to select and type "ber"
        ...
        dropdown_element_2.send_keys("ber")
...
def scrape_all_pages(start_page=1, max_pages=4):
    ...
    # Perform the initial selections (type 'r' and 'ber')
    ...
    with open("Bergi_vakances.json", "w", encoding="utf-8") as f:
